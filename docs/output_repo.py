...
os_lock_acquireLock(plock)
os_lock_releaseLock(locked_file_descriptor)
os_lock_execute(fun_run, pars, ntry = 5, plock = "tmp/plock.lock")

-------------------------methods----------------------
IndexLock.__init__(self, findex, plock)
IndexLock.get(self, val = "", ntry = 5, plock = "tmp/plock.lock")
IndexLock.put(self, val = "", ntry = 5, plock = "tmp/plock.lock")
...
utilmy\io.py
-------------------------functions----------------------
screenshot(output = 'fullscreen.png', monitors = -1)



utilmy\multithread.py
-------------------------functions----------------------
multithread_run(fun_async, input_list:list, n_pool = 5, verbose = True)
...
-------------------------functions----------------------
log(*s)
pd_text_hash_create_lsh(df, col, sep = " ", threshold = 0.7, num_perm = 10)
pd_text_getcluster(df, col, threshold, num_perm)
pd_similarity(df: pd.DataFrame, cols = [], algo = '')
test_lsh()
...
pd_to_file(df, filei, check = "check", verbose = True, **kw)
pd_read_file(path_glob = "*.pkl", ignore_index = True, cols = None, verbose = False, nrows = -1, concat_sort = True, n_pool = 1, drop_duplicates = None, col_filter = None, col_filter_val = None, dtype_reduce = None, **kw)
...
diskcache_save(df, colkey:str, colvalue:str, db_path:str = "", size_limit = 50000000000, timeout = 999, shards:int = 1)
diskcache_load(db_path = "", size_limit = 50000000000, timeout = 2, force_create = False)
to_dict(**kw)
...
is_int(x)
is_float(x)
config_load(config_path:str  =  None, path_default:str = None, config_default:dict = None)
...
os_platform_ip()
os_memory()
os_sleep_cpu(priority = 300, cpu_min = 50, sleep = 10)
os_ram_object(o, ids, hint = " deep_getsizeof(df_pd, set()
os_copy(src, dst, overwrite = False, exclude = "")
os_removedirs(path)
os_getcwd()
...
hdfs_put(from_dir = "", to_dir = "", verbose = True, n_pool = 25, dirlevel = 50, **kw)
hdfs_walk(path="hdfs = "hdfs://nameservice1/user/", dirlevel = 3, hdfs=None) = None):   ### python  prepro.py hdfs_walkimport pyarrow as pa) if hdfs is None else hdfspath = "hdfs://nameservice1/" + path if 'hdfs://' not in path else pathfdirs):)
hdfs_get(from_dir = "", to_dir = "", verbose = True, n_pool = 20, **kw)
git_repo_root()
git_current_hash(mode = 'full')
plot_to_html(dir_input = "*.png", out_file = "graph.html", title = "", verbose = False)
...
os_print_tofile(vv, file1, mode1="a") = "a"):  # print into a file='afile1, mode1) as text_file)
os_path_norm(pth):  # Normalize path for Python directoryr""" #r"D:\_devs\Python01\project\03-Connect_Java_CPP_Excel\PyBindGen\examples" """)  = = 2:)
...
str_is_azchar(x)
str_is_az09char(x)
str_reindent(s, num_spaces):  # change indentation of multine string"\n")num_spaces * " ") + line.lstrip() for line in s]s)return sdelimiters, string, maxsplit=0) = 0):  # Split into Sub-Sentenceimport remap(re.escape, delimiters))regex_pattern, string, maxsplit)sep2, ll, maxsplit=0) = 0):  # Find Sentence Patternimport re_ = maxsplitsep2)"(" + regex_pat + r")|(?:(?!" + regex_pat + ").)*", re.S)lambda m: m.group(1) if m.group(1) else "P", ll)return llx))
str_split2(delimiters, string, maxsplit=0) = 0):  # Split into Sub-Sentenceimport remap(re.escape, delimiters))regex_pattern, string, maxsplit)sep2, ll, maxsplit=0) = 0):  # Find Sentence Patternimport re_ = maxsplitsep2)"(" + regex_pat + r")|(?:(?!" + regex_pat + ").)*", re.S)lambda m: m.group(1) if m.group(1) else "P", ll)return llx))
str_split_pattern(sep2, ll, maxsplit=0) = 0):  # Find Sentence Patternimport re_ = maxsplitsep2)"(" + regex_pat + r")|(?:(?!" + regex_pat + ").)*", re.S)lambda m: m.group(1) if m.group(1) else "P", ll)return llx))
pd_str_isascii(x)
str_to_utf8(x)
...
log2(*s)
log3(*s):  ### Debuggine level 2depth = 1, lazy=True).log("DEBUG_2", ",".join([str(t) for t in s]))*s):)
logw(*s)
logc(*s)
loge(*s)
logr(*s)
...
utilmy\viz\zvizhtml2.py
-------------------------functions----------------------
log(*s)
test_getdata(verbose = True)
test1()
pd_plot_tseries_highcharts(df, coldate:str = None, date_format = '%m/%d/%Y', cols_axe1 = [], cols_axe2 = [], figsize = None, title = None, x_label = None, axe1_label = None, axe2_label = None, cfg:dict = {}, mode = 'd3', save_img = "")
pd_plot_histogram_highcharts(df:pd.DataFrame, colname: str = None, binsNumber = None, binWidth = None, title: str = "", xaxis_label: str =  "x-axis", yaxis_label: str = "y-axis", cfg:dict = {}, mode = 'd3', save_img = "", show = False)
html_show_chart_highchart(html_code, verbose = True)
html_show(html_code, verbose = True)
images_to_html(dir_input = "*.png", title = "", verbose = False)
...
date_diffstart(t) : return date_diffsecond(str_t1 = t, str_t0=t0)t)  = t0)t) :   return date_diffsecond(str_t1=t1, str_t0=t)dd)  = t)dd) :)
date_diffend(t) :   return date_diffsecond(str_t1 = t1, str_t0=t)dd)  = t)dd) :)
np_dict_tolist(dd)
np_dict_tostr_val(dd)
np_dict_tostr_key(dd)
day(s):    return int(s[8:10])s):  return int(s[5:7])s):   return int(s[0:4])s):   return int(s[11:13])cache_weekday =  {}s, fmt = 'YYYY-MM-DD', i0 = 0, i1=10) = 10):)
month(s):  return int(s[5:7])s):   return int(s[0:4])s):   return int(s[11:13])cache_weekday =  {}s, fmt = 'YYYY-MM-DD', i0 = 0, i1=10) = 10):)
year(s):   return int(s[0:4])s):   return int(s[11:13])cache_weekday =  {}s, fmt = 'YYYY-MM-DD', i0 = 0, i1=10) = 10):)
hour(s):   return int(s[11:13])cache_weekday =  {}s, fmt = 'YYYY-MM-DD', i0 = 0, i1=10) = 10):)
...
-------------------------functions----------------------
day(s):    return int(s[8:10])s):  return int(s[5:7])s):   return int(s[0:4])s):   return int(s[11:13])cache_weekday =  {}s):)
month(s):  return int(s[5:7])s):   return int(s[0:4])s):   return int(s[11:13])cache_weekday =  {}s):)
year(s):   return int(s[0:4])s):   return int(s[11:13])cache_weekday =  {}s):)
hour(s):   return int(s[11:13])cache_weekday =  {}s):)
weekday(s)
season(d)
daytime(d)
...
save_session(name = '')
isfloat(value)
isint(x): return isinstance(x, ( int, long, np.int, np.int64, np.int32 ) )))
aa_isanaconda()
aa_cleanmemory()
...
pd_save_vectopanda(vv, filenameh5):  # 'E:\_data\_data_outlier.h5'filenameh5)vv); store.append('data', pdf); store.close()filenameh5, store_id='data') = 'data'):  # 'E:\_data\_data_outlier.h5'filenameh5, store_id)    #from filereturn pdf.values   #to numpy vectorfilein1, filename, tablen='data') = 'data'):)
pd_load_panda2vec(filenameh5, store_id='data') = 'data'):  # 'E:\_data\_data_outlier.h5'filenameh5, store_id)    #from filereturn pdf.values   #to numpy vectorfilein1, filename, tablen='data') = 'data'):)
...
date_finddateid(date1, dateref)
date_alignfromdateref(array1, dateref):  #2 column array time, dataarray1)dateref)( masset, tmax), dtype = "float32")1, masset))
_date_align(dateref, datei, tmax, closei)
date_align(array1, dateref):  #2 column array time, dataarray1)dateref)( masset, tmax), dtype = "float32")1, masset))
min_withposition(values)
max_withposition(values)
_reshape(x)
_notnone(x):  return not x is Noneasset, y2 = None, y3 = None, y4 = None, y5 = None, sym = None, savename1 = '', tickperday = 20, date1 = None, graphsize = (10, 5), label = ('title', 'Time', 'Y'), legendloc = "upper left", dpi = 150, isband=0)  = 0) :)
...
pd_dataframe_toarray(df)
isfloat(value)
isint(x): return isinstance(x, ( int, long, np.int, np.int64, np.int32 ) )Xmat, type1 = "robust", type2="correl")  = "correl") :)
...
index.close(self)
index.updatehisto(self):  #Download Quotespassself) :)
index.help(self)
index.__init__(self, id1, sym, ww, tstart)
index.calc_baskettable_pct(self, type1 = "table", showdetail = 0)
index._wwpct_rebal(self, wwpct_actual, t, trebal):  # BskUnit =  1.0  !!!wwpct_th = self.wwasset[trebal, :]np.abs(wwpct_actual/wwpct_th  -1 )) > self.rebal_trigger:)
...
folioCalc._regimecalc(self, t, wwextra):  # Risk Indicatorif self.riskid  = = "spprice":   return self.riskind[0, t - 1]self.riskind[0, t - 1] / self.riskind[0, t - 1 - self.nbrange] - 1if self.riskid == "multi" = = "multi":)
...
folioRiskIndicator._regimecalc(self, t, wwextra):    #Risk Indicatorif self.riskid = = "spprice" :   return  self.riskind[0, t-1]self.riskind[0, t-1] / self.riskind[0, t-1-self.nbrange] - 1 )self.riskind, wwextra, t, self.riskind_out)self, maxiter = 1, name1 = '', isreset = 1, popsize=15)  = 15) :)
...
folioOptimizationF._regimecalc(self, t, wwextra):    #Risk Indicatorif self.riskid = = "spprice" :   return  self.riskind[0, t-1]self.riskind[0, t-1] / self.riskind[0, t-1-self.nbrange] - 1 )if self.riskid== "multi"    = = "multi"   :)
...
-------------------------functions----------------------
session_save(filename = "/folder1/name1", globals1 = None)
aa_unicode_ascii_utf8_issue()
isexist(a)
isfloat(x)
isint(x): return isinstance(x, ( int, np.int, np.int64, np.int32 ) )))
a_isanaconda()
a_run_ipython(cmd1)
a_autoreload()
a_start_log(id1 = '', folder = 'aaserialize/log/')
...
os_file_rename(some_dir, pattern = "*.*", pattern2 = "", dirlevel = 1)
os_gui_popup_show(txt)
os_print_tofile(vv, file1, mode1='a') = 'a'):  # print into a file='afile1, mode1) as text_file:  text_file.write(str(vv))pth): #Normalize path for Python directory) = =2:)
os_path_norm(pth): #Normalize path for Python directory) = =2:)
os_path_change(path1): path1 =  normpath(path1); os.chdir(path1)    #Change Working directory path): return DIRCWDfile1): return os.path.exists(file1)file1): return os.path.getsize(file1)file1):)
os_path_current(): return DIRCWDfile1): return os.path.exists(file1)file1): return os.path.getsize(file1)file1):)
os_file_exist(file1): return os.path.exists(file1)file1): return os.path.getsize(file1)file1):)
os_file_size(file1): return os.path.getsize(file1)file1):)
os_file_read(file1)
os_file_isame(file1, file2)
...
str_is_azchar(x)
str_is_az09char(x)
str_reindent(s, numSpaces): #change indentation of multine strings, '\n')numSpaces * ' ') + string.lstrip(line) for line in s]s, '\n')return sdelimiters, string, maxsplit=0) = 0):  #Split into Sub-Sentenceimport remap(re.escape, delimiters))regexPattern, string, maxsplit)sep2, ll, maxsplit=0) = 0):  #Find Sentence Patternimport resep2) '(' + regexPat + r')|(?:(?!'+ regexPat +').)*', re.S)lambda m: m.group(1) if m.group(1) else "P", ll)return llx))
str_split2(delimiters, string, maxsplit=0) = 0):  #Split into Sub-Sentenceimport remap(re.escape, delimiters))regexPattern, string, maxsplit)sep2, ll, maxsplit=0) = 0):  #Find Sentence Patternimport resep2) '(' + regexPat + r')|(?:(?!'+ regexPat +').)*', re.S)lambda m: m.group(1) if m.group(1) else "P", ll)return llx))
str_split_pattern(sep2, ll, maxsplit=0) = 0):  #Find Sentence Patternimport resep2) '(' + regexPat + r')|(?:(?!'+ regexPat +').)*', re.S)lambda m: m.group(1) if m.group(1) else "P", ll)return llx))
pd_str_isascii(x)
str_to_utf8(x)
str_to_unicode(x, encoding = 'utf-8')
...
isexist(a)
isfloat(x)
isint(x): return isinstance(x, ( int, np.int8, np.int16, np.int, np.int64, np.int32 ) )))
a_isanaconda()
os_zip_checkintegrity(filezip1)
os_zipfile(folderin, folderzipname, iscompress = True)
...
os_file_listall(dir1, pattern = "*.*", dirlevel = 1, onlyfolder = 0)
os_file_rename(some_dir, pattern = "*.*", pattern2 = "", dirlevel = 1)
os_print_tofile(vv, file1, mode1='a') = 'a'):  # print into a file='afile1, mode1) as text_file:  text_file.write(str(vv))) :)
a_get_pythonversion()
os_path_norm(pth): #Normalize path for Python directory) = =2:)
os_path_change(path1): path1 =  os_path_norm(path1); os.chdir(path1)    #Change Working directory path): return DIRCWDfile1): return os.path.exists(file1)file1): return os.path.getsize(file1)file1):)
os_path_current(): return DIRCWDfile1): return os.path.exists(file1)file1): return os.path.getsize(file1)file1):)
os_file_exist(file1): return os.path.exists(file1)file1): return os.path.getsize(file1)file1):)
os_file_size(file1): return os.path.getsize(file1)file1):)
os_file_read(file1)
...
-------------------------functions----------------------
wi(*args):  #Print with indentationaux = ''' '*INDENT)for arg in args:)
printinfile(vv, file2):  # print vvfile2, "a") as text_file:    text_file.write(vv)*args):)
wi2(*args)
indent():     global INDENT; INDENT + =  4):      global INDENT; INDENT -= 4obj):)
dedent():      global INDENT; INDENT - =  4obj):)
...
dN(d):   return    np.exp(-0.5*d*d)*ONE_SQRT_2PI   # ss.norm.pdf(d)x, y):   return  np.exp(-0.5*x*x-0.5*y*y)*ONE_2PI   # ss.norm.pdf(d)d):    return  ss.norm.cdf(d)St, K, t, T, r, d, vol))
dN2d(x, y):   return  np.exp(-0.5*x*x-0.5*y*y)*ONE_2PI   # ss.norm.pdf(d)d):    return  ss.norm.cdf(d)St, K, t, T, r, d, vol))
N(d):    return  ss.norm.cdf(d)St, K, t, T, r, d, vol))
...
bs(S0, K, t, T, r, d, vol)
bsdelta(St, K, t, T, r, d, vol, cp1): #be careful of equality for booleanSt, K, t, T, r, d, vol)-d*(T-t)) * N(d1)-d*(T-t)) * (N(d1)-1)return auxSt, K, t, T, r, d, vol, cp))
bsgamma(St, K, t, T, r, d, vol, cp)
bsstrikedelta(s0, K, t, T, r, d, vol, cp1):  #discounted risk neutral probabilitys0, K, t, T, r, d, vol)-r*T)*N(d2)-r*T)*N(-d2)return auxs0, K, t, T, r, d, vol))
...
get_all_data(file)
get_batch_data(file, index, size):#1, 5->1, 2, 3, 4, 5array = []arrayY=[]index, index+size))
get_xy(line)
file_len(fname)
feats_len(fname)
...
importFromMaven()
showLoadedClass(): #Code to see the JAR loaded.); vv =  [];):  vv.append(x.toString());return vvdir1) :)
inspectJAR(dir1)
loadSingleton(class1):  single =  jp.JClass(class1);  return Single.getInstance()x):  jp.java.lang.System.out.println(x)   #Print in Java Consolejavafile):)
java_print(x):  jp.java.lang.System.out.println(x)   #Print in Java Consolejavafile):)
...
df_topanda(vv, filenameh5, colname='data') = 'data'):  # 'E:\_data\_data_outlier.h5'filenameh5); pdf= pd.DataFrame(vv); store.append(colname, pdf); store.close()filenameh5, colname="data"):  # 'E:\_data\_data_outlier.h5'fileoutlier,colname); return pdf.values   #to numpy vectorfilein1, filename, tablen='data', lineterminator=","): #Big CSV in Datachunksize =     10 * 10 ** 6filein1, chunksize=chunksize, lineterminator=lineterminator)for chunk in list01:)
load_frompanda(filenameh5, colname="data") = "data"):  # 'E:\_data\_data_outlier.h5'fileoutlier,colname); return pdf.values   #to numpy vectorfilein1, filename, tablen='data', lineterminator=","): #Big CSV in Datachunksize =     10 * 10 ** 6filein1, chunksize=chunksize, lineterminator=lineterminator)for chunk in list01:)
csv_topanda(filein1, filename, tablen='data', lineterminator=",") = 'data', lineterminator=","): #Big CSV in Datachunksize =     10 * 10 ** 6filein1, chunksize=chunksize, lineterminator=lineterminator)for chunk in list01:)
...
max_withposition(values)
_reshape(x)
_notnone(x):  return not x is Noneasset, y2 = None, y3 = None, y4 = None, y5 = None, sym = None, savename1 = '', tickperday = 20, date1 = None, graphsize = (10, 5), label = ('title', 'Time', 'Y'), legendloc = "upper left", dpi = 150, isband=1)  = 1) :)
plot_price(asset, y2 = None, y3 = None, y4 = None, y5 = None, sym = None, savename1 = '', tickperday = 20, date1 = None, graphsize = (10, 5)
plot_priceintraday(data)
plot_pricedate(date1, sym1, asset1, sym2 = None, bsk1 = None, verticaldate = None, savename1 = '', graphsize = (10, 5)
generate_sepvertical(asset1, tt, tmax, start = None, datebar = None)
...
isfloat(value)
isint(x): return isinstance(x, ( int, long, np.int, np.int64, np.int32 ) )matx, type1 = "robust", type2="correl")  = "correl") :)
...
index.__init__(self, id1, sym, ww, tstart)
index.close()
index.updatehisto():  #Download Quotespass) :)
index.help()
index._statecalc(self): #Calculate Risk State Vector at each time treturn s1self, bsk))
...
index._weightcalc_regime2(self, wwvec, t):  #Hyper-Parameters Optimizationnbrange = self.nbrange;   nbregime= self.nbregime; masset= self.massetif t < nbrange:)
searchSimilarity.__init__(self, filejpstock=r'E = r'E:/_data/stock/daily/20160616/jp', sym01 = ['7203'], symname = ['Toyota'], startdate =  20150101, enddate = 20160601, pricetype = "close")
searchSimilarity.load_quotes_fromdb(self, picklefile = '')
searchSimilarity.__generate_return__(self, nlag)
searchSimilarity.__overweight__(self, px)
...
pathScheme_std(T, n, zz):   #Standard Path generationn+1);  sdt =  np.sqrt(T/n);  ww[0]= 01, n+1))
pathScheme_bb(T, n, zz):  #Brownian Bridge generationn); # sdt =  np.sqrt(T/n);np.round(np.log(n)  * 1.4426950408889634)) # n= 2^kmaxh= n; jmax=1T)1, kkmax+1))
pathScheme_(T, n, zz):   #Standard Path generationn+1);  sdt =  np.sqrt(T/n);  ww[0]= 01, n+1))
...
call_process(a, z, k): return np.maximum(0, np.exp(a*z)-k)@jita, z, k):  return np.piecewise(z, [np.exp(a*z) > k], [1])@jittotdim, nsample, a, strike, process01, aa = 0.25, itmax = -1, tt=10) = 10):)
binary_process(a, z, k):  return np.piecewise(z, [np.exp(a*z) > k], [1])@jittotdim, nsample, a, strike, process01, aa = 0.25, itmax = -1, tt=10) = 10):)
...
-------------------------functions----------------------
sort(x, col, asc): return   util.sortcol(x, col, asc)close, t0, t1):  return  100*( close2[:, t1] / close2[, t0] -1)tuple1):  return np.logical_and.reduce(tuple1)x, symfull=symfull)  = symfull) : return util.find(x, symfull)  # 607close, t0, t1, lag))
perf(close, t0, t1):  return  100*( close2[:, t1] / close2[, t0] -1)tuple1):  return np.logical_and.reduce(tuple1)x, symfull=symfull)  = symfull) : return util.find(x, symfull)  # 607close, t0, t1, lag))
and2(tuple1):  return np.logical_and.reduce(tuple1)x, symfull=symfull)  = symfull) : return util.find(x, symfull)  # 607close, t0, t1, lag))
ff(x, symfull=symfull)  = symfull) : return util.find(x, symfull)  # 607close, t0, t1, lag))
gap(close, t0, t1, lag)
...
-------------------------functions----------------------
spp():	print'\n\n_________________________'a0, a1 = '', a2 = '', a3 = '', a4 = '', a5 = '', a6 = '', a7 = '', a8=''  ) = ''  ):)
...
np_find_minpos(values)
np_find_maxpos(values)
date_earningquater(t1):   #JP Morgan Qearing datet1.month = =10 and t1.day >= 14) or (t1.month==1 and t1.day < 14) or t1.month in [11, 12] )
date_option_expiry(date)
linearreg(a, *args)
np_sortbycolumn(arr, colid, asc = True)
np_findlocalmax(v)
...
sort_array(vv)
save_topanda(vv, filenameh5):  # 'E:\_data\_data_outlier.h5'filenameh5)vv); store.append('data', pdf); store.close()filenameh5):  # 'E:\_data\_data_outlier.h5'fileoutlier, 'data')    #from filereturn pdf.values   #to numpy vectorxx, yy, title1="") = ""):)
load_frompanda(filenameh5):  # 'E:\_data\_data_outlier.h5'fileoutlier, 'data')    #from filereturn pdf.values   #to numpy vectorxx, yy, title1="") = ""):)
plotsave(xx, yy, title1 = "")
plotshow(xx, yy, title1 = "")
parsePDF(url)
